...
class _LocationFilterSheetState extends State<_LocationFilterSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    // Subtract keyboard height from sheet height to prevent overflow
    final sheetHeight = (mq.size.height * 0.78 - bottomInset).clamp(280.0, mq.size.height * 0.9);
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            // Search bar — visible on both pages; hint text reflects current page
            Padding(
            ...
            Expanded(
              child: PageView(
                ...
                children: [
                  ...
                  // Page 1: İlçe listesi — empty slot until city is chosen
                  ValueListenableBuilder<TextEditingValue>(
                    valueListenable: _searchCtrl,
                    builder: (_, val, __) {
        ...
        GestureDetector(
          ...
          child: Text(
            ...
            style: TextStyle(
              fontFamily: 'PlusJakartaSans',
              fontSize: 11,
              color: !onDistrictPage ? AppColors.primary : AppColors.onSurfaceVariant,
              fontWeight: !onDistrictPage ? FontWeight.w600 : FontWeight.w400,
            ),
          ),
        ),
        if (city != null) ...[
...
class _CityPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cities =
        TurkeyLocations.cities.where((c) => search.isEmpty || c.toLowerCase().contains(search.toLowerCase())).toList();
...
class _DistrictPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final districts =
        isFiltered ? allDistricts.where((d) => d.toLowerCase().contains(search.toLowerCase())).toList() : allDistricts;

    // "Tüm İlçeler" is hidden while searching
